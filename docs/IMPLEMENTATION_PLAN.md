...
- [ ] 店員購入でねこが増え、6体目以降の ×N バッジが視認できる(フェーズ5)
- [ ] マルチタッチの同時1点判定・レート制限時の演出のみ挙動(実機/自動連打)(フェーズ5)
- [ ] 画面回転・アドレスバー伸縮での ResizeObserver 追従、高DPI描画品質(フェーズ5)
- [ ] セーフエリア・ダブルタップズーム抑止の実機確認(フェーズ6)
- [ ] エッジケース通し: 時計巻き戻し・セーブ破損・旧版セーブ・8時間超放置の実ブラウザ確認(フェーズ6)
- [ ] 全店員100体状態でのレンダリング性能(フェーズ6。tick 側は単体テストで検証済み)
- [ ] `?timescale=` 加速での初回転生までの体感確認(フェーズ6)
...
**完了条件**: ブラウザ非依存の項目(バランススモーク・tick性能・README・全テスト/ビルド通過)を
満たし、ブラウザ必須の項目は保留チェックリストへ繰り延べ済みであること。
保留チェックリスト自体の消化は **ユーザー環境での受け入れ確認** として実施する
(この実行環境にブラウザがないため。議事録004・007)
